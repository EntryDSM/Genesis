...
const MinuteSelect: FC<Props> = ({ date, setFirstScheduleMinute }) => {
  ...
  const onSelectMinuteClick = (e) => {
    ...
    minute < 10
      ? setFirstScheduleMinute(0 + minute)
      : setFirstScheduleMinute(minute);
    localStorage.setItem("FirstScheduleMinute", JSON.stringify(minute));
  };

  const getLocalStorage = JSON.parse(
    localStorage.getItem("FirstScheduleMinute") ||
      JSON.stringify(
        !date
    ...
    localStorage.setItem(
      ...
      JSON.stringify(
        date
          .filter(
            (date) =>
              date.type === FIRST_ANNOUNCEMENT && date.date.slice(14, 16)
          )[0]
          .date.slice(14, 16)
      )
    );
