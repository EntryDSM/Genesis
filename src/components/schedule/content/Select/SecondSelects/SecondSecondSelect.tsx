...
const SecondSelect: FC<Props> = ({ date, setSecondScheduleSecond }) => {
  ...
  const onSelectSecondClick = (e) => {
    ...
    second < 10
      ? setSecondScheduleSecond(0 + second)
      : setSecondScheduleSecond(second);
    localStorage.setItem("secondScheduleSecond", JSON.stringify(second));
  };

  const getLocalStorage = JSON.parse(
    localStorage.getItem("secondScheduleSecond") ||
      JSON.stringify(
        !date
  ...
  React.useEffect(() => {
    localStorage.setItem(
      "secondScheduleSecond",
      JSON.stringify(
        date
          .filter(
            (date) =>
              date.type === SECOND_ANNOUNCEMENT && date.date.slice(17, 19)
          )[0]
          .date.slice(17, 19)
      )
    );
